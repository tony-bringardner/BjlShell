...
compare
	...
	public Object getValue(ShellContext sc) {
		...
		if( ctx.file_test()!=null) {
			...
			if( ctx.string().SQ_STRING()!=null) {
				String tmp = ctx.string().SQ_STRING().getText();
				ret = tmp.substring(1,tmp.length()-1);
			} else if( ctx.string().SQ_STRING()!=null) {
				ret = FileSourceShPreProcessorVisitorImpl.processString(ctx.string().SQ_STRING().getText(), sc);
			}else {
				throw new RuntimeException("Invlide string ="+ctx.string());
			}			
	...
	private Object getValue(File_testContext file_test, ShellContext sc) {
		Object ret = null;
		try {
...
True if file exists and is executable.
			 */
			case 'x':
				//TODO: True if file exists and is executable.
				break;
			/*
-G file
